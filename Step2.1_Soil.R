...
do.SoilGeology <- function(lyr, indir, vns= c('SLT', 'CLY', 'ORCDRC', 'BLD')){
  ...
  names(rl) = vns
  saveRDS(rl, file.path(dir.pihmgis, paste0('Soil_', lyr, '.RDS')))
}
do.SoilGeology(lyr='sl1', indir=file.path(dir.pihmgis, 'soil'))
do.SoilGeology(lyr='sl7', indir=file.path(dir.pihmgis, 'soil'))
